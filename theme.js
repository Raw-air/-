...
function _themeFallbackTransition(light,x,y){
  ...
  const a=ov.animate({clipPath:[_themeClip(x,y,0),_themeClip(x,y,_themeEndRadius(x,y))]},{duration:THEME_VT_DURATION,easing:'cubic-bezier(.2,.7,.2,1)',fill:'forwards'});
...
function toggleWhiteMode(el){
  ...
  try{
    ...
    transition.ready.then(()=>{
      ...
      // Belt and braces: drive the same circle through WAAPI with literal pixels. Where the
      // CSS animation already runs this is identical; where WebKit dropped it, this one shows.
      try{root.animate({clipPath:[_themeClip(x,y,0),_themeClip(x,y,radius)]},{duration:THEME_VT_DURATION,easing:'cubic-bezier(.2,.7,.2,1)',fill:'forwards',pseudoElement:'::view-transition-new(root)'});}catch(_){}
