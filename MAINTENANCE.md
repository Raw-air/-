...
正式前端為 https://raw-air.github.io/-/，Pages 來源為 `Soft-UI` 分支根目錄。此版本為資源 v99、Service Worker v59。

## 本次修正

- carousel.js (v30.5 重寫) 是「透明壓克力資料夾陣列」：DOM 仍是 app.js 的 13 本回收池 (_sfPool / sfSyncWindow / sfStudentAt 取模、無限循環)，但排列改成 perspective 1500px / perspective-origin 34% 40% + preserve-3d 的「弧形空間軌道」：一整排資料夾像抽屜裡的檔案站在一段橢圓弧上 (圓心在鏡頭這一側，弧的頂點最靠近鏡頭)，每本的位置與朝向都來自同一條曲線 rail(d)：φ = φA + d × dPhi (φA 桌機 −42° / 手機 −40°，dPhi 桌機 8° / 平板 9° / 手機 15°)，x = cx + RX·sin φ，z = zNear − RZ·(1 − cos φ) (RX 700/520/250、RZ 520/400/200、zNear 40)，yaw = atan2(RX·cos φ, −RZ·sin φ) (切線 + 90°：資料夾正面跟軌道垂直，取正面朝著鏡頭的解；>90° 就減 180°)。所以頂點左邊是正角、頂點 90° 側對鏡頭 (只看到壓克力邊緣與內頁，中間露出黑色空隙)、過了頂點是負角 (從左前方看到正面)，整條連續、沒有 sign(offset) 鏡像。側標與房號標籤放在「露出來的那一緣」：yaw>0 (頂點前) 左緣離鏡頭近 → .fd-lefty 放左邊；yaw<0 放右邊；同一本只顯示一個標籤。大小交給透視 (z 範圍約 +40 → −320，倍率 1.03 → 0.83)，manual scale 只有抽出時 ±3% 的脈衝。cx 是解出來的：讓抽出那本落在畫面寬度的 30% (手機 40%)。透明度兩端都淡到 0 才離開視窗 (回收換人不會跳出來)；模糊依深度切兩段：門檻取弧上真正到得了的深度範圍 (近端 / 遠端誰更深) 的 45% 與 75%，用抽出後的 z 判。進場 (gathered) 的起始角度取「自己那一面」的 yaw ±8°，過了頂點的資料夾才不會繞過正面閃一下。materialize 只把抽出 (ex) 歸零、鄰居的讓開 (part) 維持 1，補位時鄰居不會先合攏再打開 (ext.base / ext.basePart 各自獨立)。每本右緣有一條直式側標 (.fd-rail-label，像檔案背脊)、左右各一個房號標籤 (.fd-tab / .fd-tab-r)，抽出那本改顯示完整摘要；資料紙 (.fd-sheet) 用 --fd-counter-yaw 反轉回來正對使用者。手勢層沿用 Touch Events + Mouse Events (不用 Pointer Events，iOS Safari 在可直向捲動的頁面做水平拖曳會送 pointercancel，w3c/pointerevents#303；touchmove 只在判定為水平後才 preventDefault)，另加觸控板橫向 wheel (連續跟手，停 120ms 吸附) 與滑鼠滾輪一格一本 (頁面本身不需捲動時才接管)、方向鍵、Enter/Esc。拖曳時逐幀直接寫 transform (put() 只在值變了才寫 style)，放手用臨界阻尼彈簧 (omega 19，含初速度) 吸到最近一本；一個 rAF 迴圈 tick() 推進全部子動畫 (拖曳、彈簧、抽出、進場、開紙)，狀態機 idle / dragging / snapping / extracting / entering / locked，不用 setTimeout 疊時間軸。拖一個虛擬索引 = 中央那本在畫面上實際走的 px (measure() 用同一組投影公式算出來，桌機約 109 / 手機約 68，下限 60/54)，手指 1:1 帶著它走。DOM 回收池加大到 17 本、視窗不對稱 (近端 3 本、深處 13 本)。
- 抽出動畫 (startExtraction)：吸附完成後 ext 時間軸 0.68s — 鄰居先沿軌道讓開 0.14 本 (0–160ms，outCubic)、中央那本 60ms 後沿欠阻尼彈簧 (ζ .62、ω 13，0.3s 衝過頭 8%) 離開弧往鏡頭 +200px (平板 180 / 手機 150)、y −20、再往左 30px，朝向從軌道給的側視角 (約 56°) 校正到 22° (不轉正 — 要看起來像「從一整疊裡抽出來」，不是「中間那張轉過來面向我」)、scale 脈衝 +3%；拖曳一開始就用 160ms 把它塞回去 (pushBack，從目前值接續，不跳回 0)。進場 (sfCarousel.enter)：整排先擠在中央 (x 收到 35%、z −120、rotateY −20°、透明)，每本延遲 30ms×距離、380ms outQuint 展開，中央那本 0.36s 到位就開始抽出，總長約 1 秒；prefers-reduced-motion 直接靜態擺好。
...
- 刪除動畫 (dissolve.js)：Telegram 式高密度粉塵 + 微型黑洞吸入，用自己寫的 WebGL (兩支很小的 shader：gl.POINTS 粉塵 + 一個 quad 的黑洞)，沒有 three.js、沒有 html2canvas、不截圖。時間軸連續、沒有等待階段：0ms 資料夾 scale .985、邊緣亮一下、收掉光暈 (fd-dissolving) → 50ms 黑洞淡入 (只是改 uniform) → 80–560ms 崩解鋒面從資料夾右緣往左掃：每一格的門檻 = 離右緣的比例 + noise(x,y)·0.09，progress (easeInOut，掃到 1.14 讓雜訊最高的格子也剝離) 掃過門檻，該格的粉塵就出生在自己那一格 (一定在資料夾矩形內)；DOM 端做不到逐像素雜訊，所以每一層 (back/paper/front/edge/sheet) 套 linear-gradient(to left) 遮罩，鋒面右邊透明、左邊保留、中間 FEATHER (寬度 10%、最少 44px) 的漸層過渡帶就是正在崩解的那一帶，帶雜訊的粒子就密集出生在這條帶裡 (▓▒░) → 粉塵先在原位停 20–60ms (初速幾乎 0)，再受引力 a = GM/(d²+soft) (GM 1.8e9、soft 100²、加速度上限 14000 px/s²、速度上限 2600 px/s) 加上切向分量 30000/(d+70) 拉成弧線與螺旋 → 粒子的死法是進事件視界 (0.9 × 黑洞半徑)：判定用「這一幀走過的線段」離中心最近的距離 (快粒子一幀能走 100px，點測試會穿過去)，進去就鎖住、往中心收、先亮一下再縮到 0，不是壽命歸零 (life 2–2.6s 只是保險) → 350ms 資料夾就以「空床」長回來 (resetFields + materialize)，跟還在飛的粉塵重疊 → 存活數掉到 3% 以下 (或 1.5s) 黑洞才 scale 1 → .15 淡出，還沒進去的粉塵一起收，約 1–1.2s 結束。黑洞大小是資料夾寬度的 12% (20–52px)、位置在資料夾右上方外側 (粒子要有一段看得見的流線，不能一生出來就被吃掉)。粉塵顏色不是取樣像素，是依格子落在哪個區塊給 (玻璃白／銀／薰衣草／紫／洋紅／文字／輸入框／內頁，深淺主題兩套色)，尺寸 70% 1–2px、20% 2–3px、8% 3–4px、2% 4–5px 亮碎片。取消 (切頁、背景) 會還原遮罩、不清欄位。
- 效能：canvas、WebGL context、兩支 shader、VBO 與 4000 顆的 typed-array 粒子池全部在頁面載入時就建好，而且真的畫一次再清掉 (warmUp) — shader 這時就上 GPU 了，按下垃圾桶只是改 uniform 與開始寫 buffer，第一次刪除不會頓。目標粒子數：桌機 2400、平板 1800、手機 1300 (格距把「三成五的格子多一顆」算進去，總數才落在目標附近)，再乘上裝置學到的密度 quality (連續兩幀 >22ms 就 ×.8 或 ×.65、最低 .5，之後用黃金比例雜湊只保留 quality 比例的粒子生成；已經生出來的不動，畫面才不會突然變稀；下一場開始前回升 .12)。取樣時 opacity<.05 / hidden 的元素 (收起的摘要、側標) 不算顏色來源。跑的時候 resize 會延後到結束再補。每幀零配置：狀態放 Float32Array，只 bufferSubData 存活的那一段，一個 drawArrays(POINTS) 畫完。canvas DPR 壓在 3.2MP 以內。沒有 WebGL 就退回 Canvas 2D (同一套物理，只換畫法)。消散期間該本拿掉 backdrop-filter / 光暈 / 濾鏡 (換較實的底色)，否則每幀改遮罩都要重算毛玻璃；補位後 restore() 會把 released 設起來停掉後續遮罩寫入，長回來的資料夾才不會又被吃掉。輪播：will-change 只給離中心 −1.6~2.6 本 (fd-near)，backdrop-filter 只有 active 那本且拖曳中 (sf-moving) 關掉。實測 (headless 軟體算圖，真機有 GPU 只會更好)：手機 390×844 生出 1367 顆、桌機 1440×1000 2664 顆、WebKit 2013 顆，峰值都是全部同時存活。
- 資料夾材質：透明壓克力，亮度集中在邊緣 — 前板 rgba(255,255,255,.05~.15)、背板 .05~.09、內頁 .10 (線條 .42)、側邊 .3~.7、頂面 .18~.5、邊緣高光 1.5px .78；抽出那本前板 .10~.24 + backdrop blur 12px，紫色只在背板 ::after (.16~.30) 與 1px 邊光，沒有大片 box-shadow。層本身不設 backface-visibility:hidden (過了弧頂點的資料夾是從另一面看的)，只有側邊與頂面設 (它們靠這個切換左右哪一片可見)。字色兩個主題都是「面是透明的所以看背景」：深色主題淺字、淺色主題深字。
