...
- carousel.js 以 Touch Events (觸控) 與 Mouse Events (桌面) 驅動同一個手勢狀態機，不用 Pointer Events：iOS Safari 在可直向捲動的頁面上做水平拖曳時會送出 pointercancel (w3c/pointerevents#303)。touchmove 只在判定為水平手勢後才 preventDefault。名單無限循環 (sfStudentAt 取模、索引不設上下限；名單少於 13 人時同一人會出現在多張卡上，草稿處理見下)。卡片像在翻資料夾：位置不是「往中間擠多少」，而是由 room (中間卡左右剩下的空間) 反推每張卡的外緣要露到哪，再扣掉側轉後的半寬得出位移，所以任何螢幕寬度都排得剛好；離中心 >0.55 張就收起表單只留玻璃面與床號 (sf-sheet)，>2 張再加兩段固定模糊 (sf-blur1/2)。卡片背面是不透明的，否則後面整疊會透出來。每張卡只在值真的變了才寫 style。進出 3D 模式的 0.8 秒內改用 CSS 過渡走位 (smoothUntil)，其餘時間逐幀直接寫值才跟得上手指。
